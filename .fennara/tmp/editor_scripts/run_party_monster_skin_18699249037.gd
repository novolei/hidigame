...
func run(ctx) -> void:
	...
	_log_mesh_material(ctx, root, "default", "Hat16")
	if root.has_method("set_character_model_id"):
...
func _log_mesh_material(ctx, root: Node, label: String, mesh_name: String) -> void:
	...
	if material == null:
		...
		return
	ctx.log("%s.%s.material_class=%s name=%s" % [label, mesh_name, material.get_class(), material.resource_name])
	...
	for parameter_name in ["metallic_strength", "ao_strength", "roughness_floor", "albedo_boost"]:
