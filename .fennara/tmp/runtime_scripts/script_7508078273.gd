...
func run(ctx: Variant) -> void:
	...
	if root == null:
		ctx.close_scene()
		return
	var map_id: String = String(root.get("map_id"))
	...
	var pos: Vector3 = center + direction * max_size * 1.35
	camera.look_at_from_position(pos, center, Vector3.UP)
	await ctx.wait(0.3)
	await ctx.capture("polygon_apocalypse_%s_audit_aligned" % map_id)
	ctx.log("captured %s center=%s size=%s" % [map_id, str(center), str(bounds.size)])
	ctx.close_scene()
