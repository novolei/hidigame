...
func run(ctx: Variant) -> void:
	...
	if generated == null:
		ctx.log("generated=null root=%s" % String(root.name))
		ctx.close_scene()
		return
	...
	if max_size <= 0.01:
		max_size = 10.0
	ctx.log("bounds center=%s size=%s" % [str(center), str(bounds.size)])
	var camera: Camera3D = Camera3D.new()
	camera.name = "RuntimeAuditCamera"
	camera.fov = 45.0
	camera.near = 0.01
	camera.far = 5000.0
	root.add_child(camera)
	camera.current = true
	var directions: Array[Vector3] = [Vector3(0.85, 0.55, 0.85).normalized(), Vector3(0.85, 0.55, -0.85).normalized()]
	var labels: Array[String] = ["bunker_plus_z_audit", "bunker_minus_z_audit"]
	for i: int in range(directions.size()):
		...
		ctx.log("captured %s pos=%s" % [labels[i], str(pos)])
	ctx.close_scene()
