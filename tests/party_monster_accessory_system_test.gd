...
func _run_pickup_visual_tests(failures: Array[String]) -> void:
	...
	var initial_beacon := _find_node_by_name(pickup, "AccessoryBountyBeacon")
	_expect(initial_beacon == null, failures, "Pickup bounty beacon should be created lazily only when a local player can see it")
	...
	_expect(pickup_source.contains("IDLE_VISUAL_UPDATE_INTERVAL"), failures, "Pickup source should reduce far idle update frequency")
	_expect(pickup_source.contains("_seed_process_stagger"), failures, "Pickup source should stagger batch-spawned update timers")
	_expect(pickup_source.contains("var match_active"), failures, "Pickup source should expose a match-active gate for prep-time pre-spawn")
	_expect(pickup_source.contains("func set_match_active"), failures, "Pickup source should let Level activate pre-spawned pickups without rebuilding them")
	_expect(pickup_source.contains("not match_active or not is_available"), failures, "Pickup source should reject interaction while inactive")
	...
	_expect(level_source.contains("PARTY_MONSTER_ACCESSORY_MIN_PICKUPS := 21"), failures, "Level should spawn enough accessory pickups for replacement play")
	...
	_expect(level_source.contains("_server_spawn_match_pickups_for_round()"), failures, "Level should pre-spawn match pickups during prep")
	_expect(level_source.contains("_set_match_pickups_active(true)"), failures, "Level should activate pre-spawned pickups at PLAY start")
...
func _expect_pickup_beacon_visuals(pickup: Node, failures: Array[String]) -> void:
	var beacon := _find_node_by_name(pickup, "AccessoryBountyBeacon")
	_expect(beacon is Node3D, failures, "Pickup bounty beacon should be created after the visibility case requests it")
...
func _run_pickup_beacon_visibility_case(pickup: Node, equipped_id: String, replacement_id: String, failures: Array[String]) -> void:
	...
	_expect_pickup_beacon_visuals(pickup, failures)
