...
func _run() -> void:
	...
	if system:
		...
		_expect(battery_flashlight.get_pose_sync_interval_for_test() >= 0.045 and battery_flashlight.get_pose_sync_interval_for_test() <= 0.06, "Hunter flashlight pose sync should stay budgeted near 20Hz for responsive beam/exposure updates instead of sending every frame")
