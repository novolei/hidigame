...
func create_brush_shape_texture() -> void:
	...
	var image: Image = camera_brush.brush_shape
	image.decompress()
...
func get_atlas_textures(all_managers: Array[Node]) -> void:
	...
	for manager in all_managers:
		...
		var atlas_index := int(manager.get("atlas_index"))
		uniform.binding = atlas_index
		uniform.add_id(manager.get("atlas_texture_rid"))
		uniform_array[atlas_index] = uniform
...
func _render_callback(p_effect_callback_type: EffectCallbackType, p_render_data: RenderData) -> void:
	...
	if render_scene_buffers:
		...
		var linear_color: Color = camera_brush.color.srgb_to_linear()
